...
test('functional bug#2 grandtotal$morethan 100',async ({ page })=> {
...
test('functional bug#1 product quantity cannote update more than 2',async ({ page })=> {
await findbugspage.selectProduct(testData.functionalBugs.productSelector);
await findbugspage.addProductToCart();
...
test('functional bug #5 the twitter share button in the product details page is broken',async ({ page })=> {
await findbugspage.selectProduct(testData.functionalBugs.productSelector);
